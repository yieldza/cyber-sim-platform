...
$$('.tab').forEach(t => {
  t.onclick = () => {
    ...
    if (t.dataset.tab === 'crypto') { refreshEncryptArtifactSelect(); }
  };
});
...
// --- Encrypt / Decrypt (v0.4.5) ---
let LAST_ENCRYPTED_ID = null;

async function refreshEncryptArtifactSelect() {
  try {
    const res = await api('/files/');
    const sel = $('#encryptArtifact');
    if (!sel) return;
    sel.innerHTML = '';
    for (const a of res.artifacts) {
      if (a.file_type === 'encrypted') continue; // hide already-encrypted
      const o = document.createElement('option');
      o.value = a.id;
      o.textContent = `${a.file_type} ${a.size}B ${a.sha256.slice(0, 12)} (${a.source_op})`;
      sel.appendChild(o);
...
document.addEventListener('change', (e) => {
  if (e.target?.name === 'source' && e.target.closest('#encryptForm')) {
    const mode = e.target.value;
    $('#encSourceLibrary').classList.toggle('hidden', mode !== 'library');
    $('#encSourceUpload').classList.toggle('hidden', mode !== 'upload');
  }
});

$('#encryptUpload')?.addEventListener('change', () => {
  const f = $('#encryptUpload').files[0];
  const out = $('#encryptUploadPreview');
  if (!f) { out.textContent = ''; return; }
  if (f.size > 20 * 1024 * 1024) {
    out.innerHTML = `<span style="color:var(--accent-2)">file too large: ${(f.size/1024/1024).toFixed(1)} MB (max 20 MB)</span>`;
    return;
  }
  out.innerHTML = `selected: <code>${escapeHtml(f.name)}</code>  size=${f.size}B  type=${f.type || '?'}`;
});
...
$('#encryptForm')?.addEventListener('submit', async (e) => {
  ...
  const fd = new FormData(e.target);
  const source = fd.get('source') || 'library';
  const algo = fd.get('algo');
  $('#encryptResult').textContent = '...';
  $('#encryptActions').classList.add('hidden');

  try {
    let res;
    if (source === 'library') {
      const id = fd.get('artifact_id');
      if (!id) { $('#encryptResult').textContent = 'pick an artifact'; return; }
      res = await api(`/files/${id}/encrypt`, { method: 'POST', body: { algo } });
    } else {
      const f = $('#encryptUpload').files[0];
      if (!f) { $('#encryptResult').textContent = 'select a local file first'; return; }
      if (f.size > 20 * 1024 * 1024) {
        $('#encryptResult').textContent = 'file too large (20 MB max)';
        return;
      }
      $('#encryptResult').textContent = 'reading file…';
      const data_b64 = await fileToBase64(f);
      $('#encryptResult').textContent = 'encrypting…';
      res = await api('/files/encrypt-upload', {
        method: 'POST',
        body: { algo, data_b64, filename: f.name },
      });
    }
    LAST_ENCRYPTED_ID = res.id;
    $('#encryptResult').textContent = JSON.stringify(res, null, 2);
    $('#encryptActions').classList.remove('hidden');
    refreshLibrary();
  } catch (err) {
    $('#encryptResult').textContent = 'ERR: ' + err.message;
  }
});

document.addEventListener('click', (e) => {
  const btn = e.target.closest('button[data-dec-lang]');
  if (!btn || !LAST_ENCRYPTED_ID) return;
  const lang = btn.dataset.decLang;
  if (lang === 'enc') {
    downloadArtifact(LAST_ENCRYPTED_ID);
  } else {
    downloadDecryptor(LAST_ENCRYPTED_ID, lang);
  }
});

async function downloadDecryptor(artifactId, lang) {
  try {
    const res = await fetch(`${API}/files/${artifactId}/decryptor?lang=${lang}`, {
    ...
    const name = m ? m[1] : `decryptor.${lang}`;
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = name; a.click();
    URL.revokeObjectURL(url);
  } catch (err) {
    alert('decryptor download failed: ' + err.message);
  }
}
...
$('#decryptForm')?.addEventListener('submit', async (e) => {
  e.preventDefault();
  const fd = new FormData(e.target);
  const f = $('#decryptUpload').files[0];
  if (!f) { $('#decryptResult').textContent = 'select an encrypted file'; return; }
  if (f.size > 20 * 1024 * 1024) {
    $('#decryptResult').textContent = 'file too large (20 MB max)';
    return;
  }
  const key_b64 = (fd.get('key_b64') || '').trim();
  if (!key_b64) { $('#decryptResult').textContent = 'key_b64 required'; return; }
  const algo = fd.get('algo');
  const iv_b64 = (fd.get('iv_b64') || '').trim() || null;

  $('#decryptResult').textContent = 'decrypting…';
  try {
    const data_b64 = await fileToBase64(f);
    const res = await api('/files/decrypt-upload', {
      method: 'POST',
      body: { data_b64, algo, key_b64, iv_b64, filename: f.name },
    });
    $('#decryptResult').textContent = JSON.stringify(res, null, 2);
    if (res.id) { try { await downloadArtifact(res.id); } catch {} }
    refreshLibrary();
  } catch (err) {
    $('#decryptResult').textContent = 'ERR: ' + err.message;
  }
});
